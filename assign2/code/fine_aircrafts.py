...
train_images, test_images , train_labels, test_labels = train_test_split(images, labels_one_hot , test_size=0.08)
train_images, val_images, train_labels, val_labels =  train_test_split(train_images,train_labels, test_size=0.08)
...
import pickle
datagen_pickle = open("dg_" + typ + "_temp" + acur +".pickle","wb")
pickle.dump(datagen_test, datagen_pickle)
datagen_pickle.close()

import pickle
pickle_in = open("dg_" + typ + "_temp" + acur +".pickle","rb")
dg = pickle.load(pickle_in)

#a = model.evaluate(test_data, test_labels)

a = model.evaluate_generator( dg.flow(test_data,test_labels, batch_size=5), steps = len(test_data) / 5 )

print(a)

model.save('fine_' + typ + '_temp'  +  acur + '.h5')
